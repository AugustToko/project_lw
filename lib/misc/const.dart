import 'package:project_lw/entity/wallpaper.dart';

/// https://mgz.me/
const DEFAULT_HTML_WALLPAPER_LIST = <Wallpaper>[
  Wallpaper.all('1', WallpaperType.HTML, 'TEST1', 'description', 'author', null,
      1, 'Fuck', 'http://fff.cmiscm.com/#!/section/cylinder'),
  Wallpaper.all('2', WallpaperType.HTML, 'TEST1', 'description', 'author', null,
      1, 'Fuck', 'http://fff.cmiscm.com/#!/section/bokeh'),
  Wallpaper.all('3', WallpaperType.HTML, 'TEST1', 'description', 'author', null,
      1, 'Fuck', 'http://fff.cmiscm.com/#!/section/sheeps'),
  Wallpaper.all('4', WallpaperType.HTML, 'TEST1', 'description', 'author', null,
      1, 'Fuck', 'https://christmasexperiments.com/'),
  Wallpaper.all('5', WallpaperType.HTML, 'TEST1', 'description', 'author', null,
      1, 'Fuck', 'http://www.thewildernessdowntown.com/'),
  Wallpaper.all('6', WallpaperType.HTML, 'TEST1', 'description', 'author', null,
      1, 'AA', 'https://sandspiel.club/'),
  Wallpaper.all('7', WallpaperType.HTML, 'TEST1', 'description', 'author', null,
      1, 'AA', 'http://www.thewildernessdowntown.com/'),
  Wallpaper.all('8', WallpaperType.HTML, 'TEST1', 'description', 'author', null,
      1, 'AA', 'http://www.thewildernessdowntown.com/'),
  Wallpaper.all('9', WallpaperType.HTML, 'TEST1', 'description', 'author', null,
      1, 'AA', 'https://www.uberviz.io/'),
  Wallpaper.all('10', WallpaperType.HTML, 'TEST1', 'description', 'author',
      null, 1, 'AA', 'https://blackpawn.com/demos/vrtx-01-compo-filler/'),
  Wallpaper.all('11', WallpaperType.HTML, 'TEST1', 'description', 'author',
      null, 1, 'AA', 'http://www.aaronkoblin.com/Aaronetrope/'),
];

const PRESET_WALLPAPER_PATH = [
  'assets/demo1.lwpak',
  'assets/demo2.lwpak',
  'assets/demo3.lwpak',
];
